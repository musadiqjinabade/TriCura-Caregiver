...
class NonUrgentsScreen extends React.Component {
    state = {
        loading: true,
        data: [],
        per_page: 4,
    ...
    async getCurrentData() {
        ...
        var url = null
        // Globals.httpMode + agency_code  + Globals.domain + '/api/v1/jobs?page=' + this.state.page_no + '&per_page=' + this.state.per_page + '&job_type=assigned';
        this.state.isPetToggleSwitch?
        url = Globals.httpMode + agency_code  + Globals.domain + '/api/v1/jobs?page=' + this.state.page_no + '&per_page=' + this.state.per_page + '&job_type=assigned&is_urgent=true':
        url = Globals.httpMode + agency_code  + Globals.domain + '/api/v1/jobs?page=' + this.state.page_no + '&per_page=' + this.state.per_page + '&job_type=assigned'
        console.log("url : ", url);
        ...
        try {
            ...
            try {
                ...
                if (response.status === 200) {
                    ...
                    this.setState({
                        data: response.data.assign_shifts, loading: false, curloading: false, toggleloading: false
                    })
                }
                else {
                ...
                if (response.status === 200) {
                    if (response.data !== null) {
                        this.setState({
                            ...
                            data: this.state.data.concat(response.data.assign_shifts), loading: false, curloading: false, toggleloading: false
                        })
                    }
                    else {
                        this.state.page_no = this.state.page_no - 1;
                    ...
                    />}>
                        ...
                        <SwitchToggle containerStyle={{
                            ...
                            <TouchableOpacity onPress={this.onPress1}>
                                <Text style={[commonStyles.fs(2.2), { marginLeft: wp('2%'), color: this.state.switchOn1 ? '#4254C5' : '#828EA5' }]}>{'Show Urgent Only'}</Text>
                            </TouchableOpacity>
                    </View> */}
                        ...
                        {clientData.length>0?
                            <FlatList
                                ...
                                renderItem={({ item }) => {
                                    ...
                                    // console.log("previous_five_cgs",previous_five_cgs)
                                    ...
                                    return (
                                        ...
                                        <View style={[commonStyles.coloumn, {
                                            ...
                                            <TouchableOpacity style={[commonStyles.row, commonStyles.full, { justifyContent: 'flex-start', alignItems: 'center' }]}
                                            onPress={()=>{
                                                ...
                                                <View style={[commonStyles.full, commonStyles.row, commonStyles.mt(0.4), { flex: 1 }]}>
                                                    <View style={[commonStyles.row, commonStyles.full, { justifyContent: 'flex-start' }]}>
                                                        ...
                                                        <TouchableOpacity onPress={() => {
                                                            ...
                                                            <Text numberOfLines={2} style={[{ textAlignVertical: 'center', fontSize: hp('1.9%'), textAlign: 'left', color: '#828EA5', fontFamily: 'Catamaran-Regular', color: '#41A2EB', textDecorationLine: 'underline' }, commonStyles.ml(1)]}>{item.location}</Text>
                                                        </TouchableOpacity>
                                                    </View>
                                    ...
                                    borderColor:'#cccccc',
                                    borderBottomRightRadius:5, justifyContent: 'center', alignSelf: 'center' }} onPress={() => { this.updatedata(item.notice_id) }}>
                                    ...
                                    borderColor:'#cccccc',
                                    borderBottomRightRadius:5, justifyContent: 'center', alignSelf: 'center' }} onPress={() => { this.accept_shift(item.id, item.notice_id)}}>
...
export default connect(
    mapStateToProps,
    null
)(withNavigation(NonUrgentsScreen));