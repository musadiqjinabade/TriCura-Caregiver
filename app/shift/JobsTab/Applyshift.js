...
class Applyshift extends React.Component {
    ...
    async updatedata(){
        ...
        var access_token = this.props.data.access_token;
        var agency_code = await AsyncStorage.getItem('agency_code');
        var url = Globals.httpMode + agency_code  + Globals.domain + '/api/v1/notices/'+this.state.data.notice_id;
        console.log(url)
        return new Promise((resolve, reject) => {
            request
                .put(url)
    ...
    render() {
        ...
        if (this.state.loading) {
            ...
            // console.log("small addres:", userData.small_address)
            // console.log("caregiver:", userData_id)

            return (
                <SafeAreaView style={[commonStyles.full, commonStyles.backgroundColor]}>
                    <Header onLogo={() => { this.navigateToDashboard() }} Agency_logo={this.state.agency_details} label={"Immediate Shift"} expanded={this.state.expanded} onBack={() => { this.props.navigation.goBack() }} />
                                    ...
                                    <View style={{ margin: wp('0.5%'),flexDirection:'column' }}>
                                        ...
                                        <View style={[commonStyles.row, commonStyles.center, commonStyles.full, { justifyContent: 'flex-start'}]}>
                                            ...
                                            onPress={async () => {
                                                ...
                                                {/* <Text  style={{ flex:1,marginLeft:wp('2%'), fontSize: hp('2%'), color: '#41A2EB', textDecorationLine: this.state.data.location != null ?'underline':null, fontFamily: 'Catamaran-Regular', justifyContent: 'flex-end' }}>{this.state.data.is_previous_cg?this.state.data.location != null ? this.state.data.location : '---':userData.small_address!= null?userData.small_address:'---'}</Text> */}
                                        ...
                                        <Text style={{ marginLeft:wp('2%'), fontSize: hp('2%'), color: '#828EA5', fontFamily: 'Catamaran-Regular', justifyContent: 'flex-end' }}>Distance: {this.state.data.reimbursement_mileage !=null ?this.state.data.reimbursement_mileage.miles + ' miles' : '---'}</Text>
                                        </View>
                                        <View style={[commonStyles.row, commonStyles.center, commonStyles.full, { justifyContent: 'flex-start' }]}>
                                            <Image style={[styles.smallImage]}
                                                source={require('../../../assets/images/pay-rate.png')} />
                                        <Text style={{ marginLeft:wp('2%'), fontSize: hp('2%'), color: '#828EA5', fontFamily: 'Catamaran-Regular', justifyContent: 'flex-end' }}>Pay Rate: {this.state.data.rate !=null ?'$'+this.state.data.rate + '/hr' : '---'}</Text>
                                        </View>
