...
class ProfileScreen extends React.Component {
    ...
    edit_contact(item) {
        ...
        this.setState({ address: item.address, phone: _.get(item, 'primary_phone_number.number') ? _.get(item, 'primary_phone_number.number') : "---", city: item.city, zip_code: item.zip_code, state: item.state }, () => {
        ...
        if (this.state.loading || !this.state.data) {
            return (
                <SafeAreaView style={[commonStyles.full, commonStyles.backgroundColor]}>
                    <Header onLogo={() => { this.navigateToDashboard() }} label={'Profile'} expanded={this.state.expanded} />
                    <LoadingIndicator />
                </SafeAreaView>)
        }
        else {
            ...
            return (
                <SafeAreaView style={[commonStyles.full, commonStyles.backgroundColor]}>
                    <Header onLogo={() => { this.navigateToDashboard() }} label={'Profile'} expanded={this.state.expanded} Agency_logo={this.state.agency_details} />
                    <ScrollView showsVerticalScrollIndicator={false}
                        ...
                        <View style={[commonStyles.margin, commonStyles.column, commonStyles.full]}>
                            ...
                            <View style={[commonStyles.center]}>
                                ...
                                <TouchableOpacity style={{ justifyContent: 'flex-start', alignSelf: 'flex-end',flexDirection:'row' }}
                                    onPress={() => { this.logout(this.props.data) }}>
                                        ...
                                        source={require('../../assets/images/logout_new.png')} />
                                            <Text style={[commonStyles.headerTextBoldFont,{justifyContent:'center',alignSelf:'center', marginBottom:Platform.OS=='ios'?hp('1%'):hp('2%'), marginLeft:wp('2%')}]}>{'Log out'}</Text>
                                ...
                                <Text style={[commonStyles.headerTextBoldFont]}>{userData.name}</Text>
                            ...
                            <View style={[{ flexDirection: 'row', marginTop: wp('4%') }]} >
                                ...
                                <TouchableOpacity
                                    onPress={() => this.edit_contact(userData)}
                                    style={[commonStyles.center, { alignItems: 'flex-end', marginRight: wp('3%') }]}>
...
>
                            ...
                            <TouchableOpacity
                                ...
                                <Text style={[{ color: '#41A2EB', textDecorationLine: 'underline', marginTop: wp('2%'), fontFamily: 'Catamaran-Regular', fontSize: hp('2.2%') }]}>Change Password</Text>
                            </TouchableOpacity>
